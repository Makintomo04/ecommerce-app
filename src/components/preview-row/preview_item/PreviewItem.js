...
const Preview_Item = ({
  ...
  return (
    <div className="preview_item">
      ...
      <div
        className="preview_item__image"
        style={{ backgroundImage: `url(${imageUrl})` }}
      ></div>
      <div className="preview_item__details-row">
        <div className="preview_item__details">
          <span className="preview_item__details__brand">{brand}</span>
          <span className="preview_item__details__name">{name}</span>
          <span className="preview_item__details__price">£{price}.00</span>
        </div>
        {!wishlist && <img src={AddToBag} onClick={() => moveToBag(item)} />}
      </div>
      {wishlist && (
        <span
          className="preview_item__move-to-bag"
...
export default connect(mapStateToProp, mapDispatchToProps)(Preview_Item);
